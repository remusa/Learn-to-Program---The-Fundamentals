...
EXPECTED_FNS = [
    'hours_difference', 'to_float_hours', 'get_hours', 'get_minutes',
    'get_seconds', 'time_to_utc', 'time_from_utc'
]
...
class Clock(object):
    ...
    def refresh(self, forced=False):
        ...
        self.canvas.create_oval(
            10, 10, self.width - 10, self.height - 10, width=2, fill="#334455")
...
if __name__ == '__main__':
    ...
    for x in range(4):
        canvases.append(
            Clock(
                frame,
                zone=['local', 'UTC-4.0', 'UTC+0.0', 'UTC+12.0'][x],
                row=0,
                column=x))
    ...
    tkinter.Button(
        frame, text='Slow motion', command=slow).grid(
            row=3, column=0)
    ...
    tkinter.Button(
        frame, text='Normal speed', command=normal_speed).grid(
            row=3, column=1)
    ...
    # function as the one to be called when the button is clicked.
    tkinter.Button(
        frame, text='Fast forward', command=speed).grid(
            row=3, column=2)
